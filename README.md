...
Weather App - веб-приложение для просмотра текущей погоды, разработанное на React. Позволяет искать погоду по названию города или определять её автоматически по геолокации. Данные получаются в реальном времени через OpenWeatherMap API и отображаются в виде карточек с температурой, влажностью, ветром и давлением.

![demo](docs/demo.gif)

## Use-case

![use-case](docs/use-case-diagram.drawio.png)
